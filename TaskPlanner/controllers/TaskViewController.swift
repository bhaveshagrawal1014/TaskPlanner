...
class TaskViewController: UIViewController {
    ...
    let descriptionPromptText = "Brief description of the task"
    ...
    func showTaskData() {
        ...
        if mode == .edit, let task = taskToEdit {
            ...
            adressLabel.text = task.address?.formattedAddress ?? "Not selected"
    ...
    @IBAction func backFromMap(segue: UIStoryboardSegue) {
        if let vc = segue.source as? MapViewController {
            ...
            adressLabel.text = currentAddress?.formattedAddress ?? "Not selected"
        }
    }
