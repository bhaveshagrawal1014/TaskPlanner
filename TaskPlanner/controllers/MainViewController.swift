...
extension MainViewController: FilterDelegate {
    ...
    func filterDidChanged(filterValue: TaskFilterType) {
        ...
        if filterValue == .unchecked {
            ...
            emptyLabel.text = "There are no upcoming tasks. \n Press “+” to add a new task."
        } else {
            fetchController?.fetchRequest.predicate = completedPredicate
            emptyLabel.text = "There are no completed tasks. \n Press “+” to add a new task."
        }
...
extension MainViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let removeAction = UITableViewRowAction(style: .destructive, title: "Delete", handler: removeRow)
        return [removeAction]
    }
